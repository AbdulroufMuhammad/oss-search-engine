...
results, normalizes them, and enriches them with deterministic ranking
metadata, query analysis, source extraction, finance lookups, and event
detection.
...
Pure local query analysis — no SearXNG call, so it works even if the
...
Degrades gracefully if SearXNG is unreachable: ``200`` with empty
...
- ``filings``
- ``news`` (from SearXNG — degrades to an empty list if unreachable)
- ``response_time``
...
Unlike research/finance, this one has no fallback path: a SearXNG outage
