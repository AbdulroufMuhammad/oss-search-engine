...
async def search(request: Request, response: Response, q: str, max_results: int = 10):
    if not q.strip():
        raise HTTPException(status_code=400, detail="q must not be empty")
    max_results = max(1, min(50, max_results))

    cached = cache.get(q, max_results)
    ...
    try:
        result = await provider.search(q, max_results=max_results)
    except ProviderUnavailableError as exc:
        raise HTTPException(status_code=502, detail=str(exc)) from exc

    cache.set(q, max_results, result)
